(function() {
  ...
  // Modo competitivo: la web SIEMPRE carga en default (modo normal).
  // El boton alterna el modo durante la sesion, pero NO se recuerda entre visitas.
  const modeBtn = document.getElementById('modeBtn');
  safeStorage.removeItem('amy-comp-mode');
  ...
  const agentBgImages = {
    ...
    gekko: 'assets/imagenes/gekko_art.avif',
    brimstone: 'assets/imagenes/brimstone_art.avif'
  };
  const agentColors = {
    sage: '#3b82f6',
    gekko: '#4ade80',
  ...
  agentCards.forEach(card => {
    ...
    card.addEventListener('mouseenter', () => {
      if (agent && agentColors[agent]) {
        ...
        if (agentBgImages[agent] && agentBgOverlay) {
  ...
  abilityButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (abilitiesData[agent] && abilitiesData[agent][key]) {
        if (nameEl) nameEl.textContent = abilitiesData[agent][key].name;
        if (descEl) descEl.textContent = abilitiesData[agent][key].desc;
      }
    });
  });
  ...
  if (canvas) {
    ...
    function updateStats() {
      if (aimScoreVal) aimScoreVal.textContent = hits;
      const accuracy = totalClicks > 0 ? Math.round((hits / totalClicks) * 100) : 100;
      if (aimAccuracyVal) aimAccuracyVal.textContent = accuracy + "%";
      if (aimTimeVal) aimTimeVal.textContent = timeLeft + "s";
    }
    ...
    function startGame() {
      ...
      totalClicks = 0;
      timeLeft = 10;
      if (aimStats) aimStats.style.visibility = 'visible';
      spawnTarget();
      updateStats();
      if (startAimBtn) startAimBtn.textContent = "Detener Juego ⏹️";
    ...
    function endGame() {
      ...
      if (startAimBtn) startAimBtn.textContent = "Iniciar Juego 🎯";
      if (aimStats) aimStats.style.visibility = 'hidden';
      setTimeout(() => {
        if (!animFrameId) draw();
      }, 200);
    }
  ...
  // Intersection Observer para secciones con fade-in
