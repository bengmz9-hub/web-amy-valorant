/* Strands (reactbits) portado a vanilla JS para AMY WEB PROJECT.
   Hebras de luz animadas (shader OGL WebGL2) detrás de la tarjeta SAGE.
   Original: reactbits.dev/Strands (componente React + OGL). */
import { Renderer, Program, Mesh, Color, Triangle, RenderTarget } from './ogl.esm.js';
...
function initStrands(ctn) {
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uColors: { value: buildPalette(STRANDS_CONFIG.colors) },
      uColorCount: { value: Math.min(STRANDS_CONFIG.colors.length, MAX_COLORS) },
      uStrandCount: { value: Math.min(STRANDS_CONFIG.count, MAX_STRANDS) },
      ...
      uHueShift: { value: STRANDS_CONFIG.hueShift },
  ...
  const glassProgram = new Program(gl, {
    ...
    uniforms: {
      ...
      uResolution: { value: [ctn.offsetWidth, ctn.offsetHeight] },
      uRadius: { value: 0.46 * STRANDS_CONFIG.glassSize },
      uRefraction: { value: STRANDS_CONFIG.refraction },
      uDispersion: { value: STRANDS_CONFIG.dispersion }
    }
  });
  ...
  const update = t => {
    ...
    if (STRANDS_CONFIG.glass) {
      renderer.render({ scene: mesh, target: renderTarget });
      glassProgram.uniforms.uScene.value = renderTarget.texture;
      glassProgram.uniforms.uRefraction.value = STRANDS_CONFIG.refraction;
      glassProgram.uniforms.uDispersion.value = STRANDS_CONFIG.dispersion;
      glassProgram.uniforms.uRadius.value = 0.46 * STRANDS_CONFIG.glassSize;
